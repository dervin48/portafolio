...
class ControladorVentas{
	...
	static public function ctrDescargarXML() {
		if (isset($_GET["xml"])) {
			...
			// Abrir el archivo XML para escritura
			...
			// Comenzar el elemento raíz del XML
			$objetoXML->startElement("fe:Invoice");
	
			// Agregar atributos al elemento raíz si es necesario
			$objetoXML->writeAttribute("xmlns:fe", "http://www.dian.gov.co/contratos/facturaelectronica/v1");
			// Agregar otros atributos según corresponda
	
			// Crear elementos hijos en el XML
			foreach ($listaProductos as $key => $value) {
				$objetoXML->startElement("Product");
				$objetoXML->writeElement("Description", $value["descripcion"]);
